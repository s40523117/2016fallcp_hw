...
使用num = random.randint(X,Y)【X Y=要帶入的數字區間】，隨機選出所設定區間的數字，再利用迴圈設
定邀選區幾個數字，但所選的數字可能重複，若使用random.sample(X,Y)，使其產生不同的是字亂碼。
猜數字遊戲：
        將
            if 標準答案 < 你猜的數字:
                #print("太大了，再猜一次 :)加油")
 ...
 用以上程式進行手動的猜選數字，由程式判別所猜選的數字太大或太小，將程式中【你猜的數字】改成【PC猜的數字】則會由電腦自動進行猜測。
